-- =============================================================================
-- Prepare Data for Tableau Dashboard (SQL Version)
...
-- Option A: If you have a transactions table with predictions
CREATE OR REPLACE VIEW tableau_fraud_data AS
SELECT 
    -- Basic identifiers
    t.transaction_id,
    t.customer_id,
...
CREATE OR REPLACE VIEW tableau_summary_stats AS
SELECT 
...
CREATE OR REPLACE VIEW tableau_psi_data AS
WITH baseline AS (
...
SELECT 
    ...
    CASE 
        ...
        ELSE (COALESCE(c.actual_pct, 0) - COALESCE(b.expected_pct, 0)) * 
             LOG(COALESCE(c.actual_pct, 0) / NULLIF(COALESCE(b.expected_pct, 0), 0))
    END AS psi_component,
    CAST(GETDATE() AS DATE) AS psi_date
FROM baseline b
FULL OUTER JOIN current_period c ON b.prob_bin = c.prob_bin;

-- Calculate total PSI
CREATE OR REPLACE VIEW tableau_psi_score AS
SELECT 
    psi_date,
...
CREATE OR REPLACE VIEW tableau_queue_overview AS
SELECT 
    queue_status,
    decision_priority,
    analyst_name,
    ...
    -- Age metrics
    AVG(DATEDIFF(hour, queue_created_date, GETDATE())) AS avg_wait_hours,
    MAX(DATEDIFF(hour, queue_created_date, GETDATE())) AS max_wait_hours,
...
CREATE OR REPLACE VIEW tableau_feature_psi AS
...
FULL OUTER JOIN current_amount c ON b.amount_bin = c.amount_bin;

-- Aggregate feature PSI scores
CREATE OR REPLACE VIEW tableau_feature_psi_scores AS
SELECT 
    feature_name,
    psi_date,
...
-- In Tableau, connect to your database and use these views:
